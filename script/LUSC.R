...
df <-
  data.frame(
    ...
    c(
      ...
      rep("only_1_complexity", length(only_1_complexity)),
...
compar <-
  list(
    c("NonTDP_group_complexity", "only_1_complexity"),
    c("NonTDP_group_complexity", "only_2_complexity"),
    c("NonTDP_group_complexity", "only_3_complexity")
  )
p <-
  ggboxplot(
    df,
...
LUSC_clin <-
  read.table(
    file = "TCGA-LUSC.GDC_phenotype.txt",
    sep = "\t",
    header = T,
    na.strings = c("", "NA"),
    quote = ''
...
new_final_group <-
  final_group[intersect(rownames(new_LUSC_clin), names(final_group))]
...
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage iib"] <-
  "Stage II"
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage iii"] <-
  "Stage III"
...
########prognosis
new_LUSC_clin$RFS <-
  new_LUSC_clin$days_to_new_tumor_event_after_initial_treatment
new_LUSC_clin$RFS_event <-
  new_LUSC_clin$new_tumor_event_after_initial_treatment
new_LUSC_clin <-
  new_LUSC_clin[complete.cases(new_LUSC_clin$RFS_event), ]
for (i in 1:dim(new_LUSC_clin)[1]) {
  if (new_LUSC_clin$RFS_event[i] == 'NO') {
    new_LUSC_clin$RFS[i] <-
      new_LUSC_clin$days_to_last_follow_up.diagnoses[i]
  }
}
...
setwd(working_dictory)
all_cnv <-
  read.table(file = "TCGA-LUSC.masked_cnv.txt", sep = "\t", header = T)
sample_name <- unique(all_cnv$sample)
sample_cnv <- list()##CNV information for each sample
for (i in 1:length(sample_name)) {
  sample_cnv[[i]] <- all_cnv[all_cnv$sample == sample_name[i], ]
...
rownames(effct_matrix) <-
  c("EGFR", "NFE2L2", "MLLT11", "NBPF10", "PSPH", "NOTCH2")
...
UBE2QL1_chr <- "5"
UBE2QL1_group_1 <-
  UCSC_paragraph(only_2_3_chr_cnv, UBE2QL1, UBE2QL1_chr)
...
group_1_mut <-
  intersect(unique(tumor_sample_1), group_1)##A classified sample with variation
...
setwd(working_dictory)
LUSCgroup_1 <-
  read.table(file = "group_1_mutation_gene.txt", sep = "\t", header = T)
LUSConly_2_3 <-
...
group_1_mutation <-
  c(intersect(LUSCgroup_1[, 1], OGgene[, 1]),
    intersect(LUSCgroup_1[, 1], TSGgene[, 1]))
only_2_3_mutation <-
  c(intersect(LUSConly_2_3[, 1], OGgene[, 1]),
    intersect(LUSConly_2_3[, 1], TSGgene[, 1]))
...
####################################
new_genes_tumor_mutation <-
  read.table(
    "genes_tumor_mutation.txt",
    header = T,
    row.names = 1,
    sep = "\t",
    quote = "",
...
deg_matrix <-
  pro_exp[, c(intersect(only_1, colnames(pro_exp)), intersect(only_2_3, colnames(pro_exp)))]
...
up_genes <-
  rownames(DEG_voom[with(DEG_voom, (P.Value < p_value &
...
setwd(working_dictory)
options(stringsAsFactors = F)
LUSC_sample <- read.table(file = "LUSC_sample.txt", sep = "\t", header = T)
all_sample_cnv <-
  read.table(file = "CCLE_copynumber_2013-12-03.seg.txt", sep = "\t", header = T)
LUSC_cnv <-
  all_sample_cnv[all_sample_cnv$CCLE_name %in% LUSC_sample$CCLE.name, ]
colnames(LUSC_cnv) <-
...
setwd(working_dictory)
options(stringsAsFactors = F)
LUSC_drug <- read.table(file = "LUNG_drug.txt", sep = "\t", header = T)
load("patient_group.Rdata")
drug_list <- list()
for (i in 1:length(unique(LUSC_drug$Compound))) {
  drug_list[[i]] <-
    LUSC_drug[LUSC_drug$Compound == unique(LUSC_drug$Compound)[i], ]
}
