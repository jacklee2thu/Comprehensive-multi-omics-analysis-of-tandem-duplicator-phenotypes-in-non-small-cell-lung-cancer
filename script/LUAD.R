...
all_cnv <-
  read.table(file = "TCGA-LUAD.masked_cnv.txt", sep = "\t", header = T)
...
all_sample_chr_cv <-
  list()###Record the CNV information of each chromosome of each patient, whether there is TD
for (w in 1:length(sample_cnv)) {
  sample_chr_cnv <-
    list()##CNV information for each chromosome of each patient
  for (j in 1:length(chr_name)) {
    sample_chr_cnv[[j]] <-
      sample_cnv[[w]][sample_cnv[[w]]$Chrom == chr_name[j], ]
  }
  ...
  for (z in 1:length(sample_chr_cnv)) {
    if (dim(sample_chr_cnv[[z]])[1] > 1) {
      ...
      for (i in 1:(dim(sample_chr_cnv[[z]])[1] - 1)) {
        if (all(
          sample_chr_cnv[[z]]$End[i] - sample_chr_cnv[[z]]$Start[i] >= 100,
          sample_chr_cnv[[z]]$value[i] > 0,
          sample_chr_cnv[[z]]$value[i] - max(sample_chr_cnv[[z]]$value[i - 1], sample_chr_cnv[[z]]$value[i +
                                                                                                         1]) > 0.3,
          abs(sample_chr_cnv[[z]]$value[i - 1] - sample_chr_cnv[[z]]$value[i + 1]) <=
          0.3
        )) {
          TD_likegroup[i] <- 1
        } else{
      ...
      if (all(
        sample_chr_cnv[[z]]$End[dim(sample_chr_cnv[[z]])[1]] - sample_chr_cnv[[z]]$Start[dim(sample_chr_cnv[[z]])[1]] >=
        100,
        sample_chr_cnv[[z]]$value[dim(sample_chr_cnv[[z]])[1]] > 0,
        sample_chr_cnv[[z]]$value[dim(sample_chr_cnv[[z]])[1]] - sample_chr_cnv[[z]]$value[dim(sample_chr_cnv[[z]])[1] -
                                                                                           1] > 0.3,
        abs(sample_chr_cnv[[z]]$value[dim(sample_chr_cnv[[z]])[1] - 1]) <= 0.3
      )) {
        TD_likegroup[dim(sample_chr_cnv[[z]])[1]] <- 1
      } else{
        TD_likegroup[dim(sample_chr_cnv[[z]])[1]] <- 0
      }
      sample_chr_cnv[[z]]$TD_likegroup <- TD_likegroup
    } else{
      sample_chr_cnv[[z]]$TD_likegroup <- 0
    }
  }
...
for (i in 1:length(all_sample_chr_cv)) {
  ...
  for (j in 1:length(all_sample_chr_cv[[i]])) {
    TD_frame <-
      rbind(TD_frame, all_sample_chr_cv[[i]][[j]], stringsAsFactors = F)
  }
...
for (i in 1:length(all_sample_chr_cv)) {
  ...
  for (j in 1:length(chr_name)) {
    chr_abs[j] <-
      abs(sum(all_sample_chr_cv[[i]][[j]]$TD_likegroup) - chr_mean[j])
  }
...
mixmdl = normalmixEM(
  TDP_score,
  mu = re_TDP_score$parameters$mean,
  sigma = re_TDP_score$parameters$variance$sigmasq,
  epsilon = 1e-3
)
plot(mixmdl, which = 2)
...
mixmdl = normalmixEM(
  all_TD_size,
  mu = span_size_distri$parameters$mean,
  sigma = span_size_distri$parameters$variance$sigmasq,
  epsilon = 1e-3
)
plot(mixmdl, which = 2)
...
class_0 <-
  range(all_TD_size[span_size_distri$classification == 1])##-1~0.414
class_1 <-
  range(all_TD_size[span_size_distri$classification == 1])##0.416~0.846
class_2 <-
  range(all_TD_size[span_size_distri$classification == 2])##1.85~2.71
class_3 <-
  range(all_TD_size[span_size_distri$classification == 3])##2.71~3.85

group_list <- list()
for (i in 1:length(span_size_)) {
  group_list[[i]] <-
    c(length(which(
      class_1[1] < span_size_[[i]] &
        span_size_[[i]] < class_1[2]
    )), length(which(
      class_2[1] < span_size_[[i]] &
        span_size_[[i]] < class_2[2]
    )), length(which(
      class_3[1] < span_size_[[i]] & span_size_[[i]] < class_3[2]
    )))
}
...
sample_sum <-
  apply(group_set, 2, sum)#####The sum of 1,2, and 3 classes per patient
names(sample_sum) <- colnames(group_set)

###A proportional matrix, using 1/4 to determine which group you belong to
new_group_list <-
  matrix(0, nrow = dim(group_set)[1], ncol = dim(group_set)[2])
...
group_1_2 <-
  colnames(new_group_list[, new_group_list[1, ] >= 1 / 4 &
                            new_group_list[2, ] >= 1 / 4])
group_1_3 <-
  colnames(new_group_list[, new_group_list[1, ] >= 1 / 4 &
                            new_group_list[3, ] >= 1 / 4])
group_2_3 <-
  colnames(new_group_list[, new_group_list[2, ] >= 1 / 4 &
                            new_group_list[3, ] >= 1 / 4])
...
only_1_2 <-
  setdiff(group_1_2, intersect(intersect(group_1, group_2), group_3))
only_1_3 <-
  setdiff(group_1_3, intersect(intersect(group_1, group_2), group_3))
only_2_3 <- group_2_3
...
df <-
  data.frame(
    ...
    c(
      rep("NonTDP_group_complexity", length(NonTDP_group_complexity)),
      rep("only_1_2_complexity", length(only_1_2_complexity)),
      rep("only_1_3_complexity", length(only_1_3_complexity)),
      rep("only_2_complexity", length(only_2_complexity)),
      rep("only_3_complexity", length(only_3_complexity)),
...
LUAD_clin <-
  read.table(
    file = "TCGA-LUAD.GDC_phenotype.txt",
    sep = "\t",
    header = T,
    na.strings = c("", "NA"),
    quote = ''
...
new_final_group <-
  final_group[intersect(rownames(new_LUAD_clin), names(final_group))]
...
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage ia"] <-
  "Stage I"
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage ib"] <-
  "Stage I"
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage iia"] <-
  "Stage II"
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage iib"] <-
  "Stage II"
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage iiia"] <-
  "Stage III"
tumor_stage.diagnoses[tumor_stage.diagnoses == "stage iiib"] <-
  "Stage III"
tumor_stage.diagnoses[tumor_stage.diagnoses == "not reported"] <- NA
barplot(table(new_final_group, tumor_stage.diagnoses), col = 2:6)
barplot(table(new_final_group, primary_therapy_outcome_success),
        col = 2:6)
...
########prognosis
new_LUAD_clin$RFS <-
  new_LUAD_clin$days_to_new_tumor_event_after_initial_treatment
new_LUAD_clin$RFS_event <-
  new_LUAD_clin$new_tumor_event_after_initial_treatment
new_LUAD_clin <-
  new_LUAD_clin[complete.cases(new_LUAD_clin$RFS_event), ]
for (i in 1:dim(new_LUAD_clin)[1]) {
  if (new_LUAD_clin$RFS_event[i] == 'NO') {
    new_LUAD_clin$RFS[i] <-
      new_LUAD_clin$days_to_last_follow_up.diagnoses[i]
  }
}
...
attach(new_LUAD_clin)
ovsur <-
  Surv(RFS, as.numeric(RFS_event))                                            #################建立生存对象
sur_stat <- summary(coxph(ovsur ~ new_LUAD_clin$new_final_group))
srisk <-
  cbind(as.numeric(RFS_event),
        as.numeric(RFS),
        new_LUAD_clin$new_final_group)
...
risk.fit <-
  survfit(Surv(as.numeric(srisk$days / 30), as.numeric(srisk$status)) ~ srisk$group)
t_results <-
  survdiff(Surv(as.numeric(srisk$days / 30), as.numeric(srisk$status)) ~ srisk$group)
...
plot(
  risk.fit,
  col = c(2, 4, 5, 6),
  main = paste("Survival analysis based on TDP signature, ", "p value=", p, sep =
                 ""),
  xlab = 'Time (months)',
...
UCSC_paragraph <- function(chr_cnv, GENE, chrname) {
  invest_cnv <-
    list()#######Stores the data frames involved in each OG/TSG
  for (j in 1:length(chr_cnv)) {
    start_row <- c()
    end_row <- c()
    for (i in 1:dim(chr_cnv[[j]][[chrname]])[1]) {
      if (chr_cnv[[j]][[chrname]][i, ]$Start <= GENE[1] &
          GENE[1] <= chr_cnv[[j]][[chrname]][i, ]$End) {
        start_row <- i
      }
      if (chr_cnv[[j]][[chrname]][i, ]$Start <= GENE[2] &
          GENE[2] <= chr_cnv[[j]][[chrname]][i, ]$End) {
        end_row <- i
      }
    }
  ...
  final_frame <-
    data.frame()########Merge the matrices involved in OG/TSG
...
test_group <-
  list(MAPK1_group_1, MLLT11_group_1, TMCO2_group_1, EXO5_group_1)
...
for (i in 1:dim(effct_matrix)[2]) {
  ...
  if (all(!any(effct_matrix[, i] %in% 1),
          !any(effct_matrix[, i] %in% 4),
          any(effct_matrix[, i] %in% 2))) {
    final_effect[i] <- 2
  }
  if (all(!any(effct_matrix[, i] %in% 1),
          !any(effct_matrix[, i] %in% 4),
          any(effct_matrix[, i] %in% 3))) {
    final_effect[i] <- 3
  }
  if (all(
    !any(effct_matrix[, i] %in% 1),
    !any(effct_matrix[, i] %in% 4),
    !any(effct_matrix[, i] %in% 2),
    !any(effct_matrix[, i] %in% 3)
  )) {
...
pheatmap(
  ...
  cutree_rows = dim(final_effect)[1],
  show_colnames = F,
  treeheight_row = 0,
  treeheight_col = 0,
  cellheight = 20
...
CAPRIN2_chr <- "12"
CAPRIN2_group_1 <-
  UCSC_paragraph(only_2_3_chr_cnv, CAPRIN2, CAPRIN2_chr)
...
test_group <-
  list(KRAS_group_1, EGFR_group_1, CAPRIN2_group_1, KMT2B_group_1)
names(test_group) <- rownames(effct_matrix)
#OG
for (i in 1:3) {
  for (j in 1:dim(test_group[[i]])[1]) {
...
group_1 <- unique(c(only_1, only_1_2, only_1_3))
tumor_sample <-
  read.table(file = "tumor_sample.txt", header = T, sep = "\t")####all samples
...
only_2index <-
  c()#######Indicates which ordinals are classified samples
...
only_2index_frame[, 2] <-
  tumor_sample[, 1]#########1Representation classification sample
write.table(
  only_2index_frame,
  file = "only_2index_frame.txt",
  col.names = F,
  row.names = F,
  sep = "\t",
  quote = F
...
only_2_3index_frame[, 2] <-
  tumor_sample[, 1]#########1Representation classification sample
write.table(
  only_2_3index_frame,
  file = "only_2_3index_frame.txt",
  col.names = F,
  row.names = F,
  sep = "\t",
  quote = F
...
setwd(working_dictory)
LUADonly_2 <-
  read.table(file = "only_2_mutation_gene.txt", sep = "\t", header = T)
LUADonly_2_3 <-
  read.table(file = "only_2_3_mutation_gene.txt", sep = "\t", header = T)


OGgene <- read.table(file = "oncogene.txt", sep = "\t", header = T)
TSGgene <- read.table(file = "TSG.txt", sep = "\t", header = T)
only_2_mutation <-
  c(intersect(LUADonly_2[, 1], OGgene[, 1]),
    intersect(LUADonly_2[, 1], TSGgene[, 1]))
only_2_3_mutation <-
  c(intersect(LUADonly_2_3[, 1], OGgene[, 1]),
    intersect(LUADonly_2_3[, 1], TSGgene[, 1]))
...
for (i in 1:length(final_gene)) {
  ...
  KEGG_gene <-
    enrichKEGG(final_gene_entriz,
               organism = "human",
               pvalueCutoff = 0.05)
  GO_gene <-
    enrichGO(final_gene_entriz,
             'org.Hs.eg.db',
             ont = "BP",
             pvalueCutoff = 0.05)
...
new_kegg <-
  data.frame(all_path[path_index, c(2, 5, 8, 9)], group = rep(i, length(observed_path)))
#all_kegg<-new_kegg
...
final_frame$Description <-
  factor(final_frame$Description, levels = rev(observed_path))
...
new.mutations_1 <-
  new.mutations[which(new.mutations$Hugo_Symbol %in% genes), ]
...
new.mutations_1[which(new.mutations_1[, 2] == "Missense_Mutation"), 2] <-
  "Missense"
new.mutations_1[which(new.mutations_1[, 2] == "Nonsense_Mutation"), 2] <-
  "Nonsense"
new.mutations_1[which(new.mutations_1[, 2] == "Splice_Region"), 2] <-
  "Splice Site"
new.mutations_1[which(new.mutations_1[, 2] == "Splice_Site"), 2] <-
  "Splice Site"
new.mutations_1[which(new.mutations_1[, 2] == "Frame_Shift_Ins"), 2] <-
  "Frame Shift"
new.mutations_1[which(new.mutations_1[, 2] == "Frame_Shift_Del"), 2] <-
  "Frame Shift"
new.mutations_1[which(new.mutations_1[, 2] == "In_Frame_Del"), 2] <-
  "In frame indel"
new.mutations_1[which(new.mutations_1[, 2] == "In_Frame_Ins"), 2] <-
  "In frame indel"
new.mutations_1[which(new.mutations_1[, 2] == "3'UTR"), 2] <- "Syn"
new.mutations_1[which(new.mutations_1[, 2] == "5'UTR"), 2] <- "Syn"
new.mutations_1[which(new.mutations_1[, 2] == "3'Flank"), 2] <-
  "Syn"
new.mutations_1[which(new.mutations_1[, 2] == "5'Flank"), 2] <-
  "Syn"
new.mutations_1[which(new.mutations_1[, 2] == "Intron"), 2] <- "Syn"
new.mutations_1[which(new.mutations_1[, 2] == "Translation_Start_Site"), 2] <-
  "Other non syn"
new.mutations_1[which(new.mutations_1[, 2] == "Nonstop_Mutation"), 2] <-
  "Other non syn"
new.mutations_1[which(new.mutations_1[, 2] == "Silent"), 2] <-
  "Other non syn"

genes_tumor_mutation <-
  matrix(0, nrow = length(genes), ncol = length(all_tumor))
rownames(genes_tumor_mutation) <- genes
colnames(genes_tumor_mutation) <- all_tumor
for (gene in genes) {
  for (tumor in all_tumor) {
    new.mutations_2 <-
      filter(new.mutations_1,
             Hugo_Symbol == gene & Tumor_Sample_Barcode == tumor)
    if (dim(new.mutations_2)[1] >= 1) {
      for (i in 1:dim(new.mutations_2)[1]) {
        genes_tumor_mutation[new.mutations_2$Hugo_Symbol[i], tumor] <-
          unique(new.mutations_2$Variant_Classification[i])
      }
    }
  }
}
write.table(genes_tumor_mutation,
            file = "genes_tumor_mutation.txt",
            quote = F,
            sep = "\t")
...
Variant_Classification <-
  unique(new.mutations_1$Variant_Classification)
new_genes_tumor_mutation_num <-
  matrix(0, nrow = length(genes), ncol = length(all_tumor))
new_genes_tumor_mutation_num[which(new_genes_tumor_mutation == "Syn")] = 1
new_genes_tumor_mutation_num[which(new_genes_tumor_mutation == "Missense")] <-
  2
new_genes_tumor_mutation_num[which(new_genes_tumor_mutation == "Splice Site")] <-
  3
new_genes_tumor_mutation_num[which(new_genes_tumor_mutation == "Nonsense")] <-
  4
new_genes_tumor_mutation_num[which(new_genes_tumor_mutation == "Frame Shift")] <-
  5
new_genes_tumor_mutation_num[which(new_genes_tumor_mutation == "In frame indel")] <-
  6
new_genes_tumor_mutation_num[which(new_genes_tumor_mutation == "Other non syn")] <-
  7
rownames(new_genes_tumor_mutation_num) <-
  rownames(new_genes_tumor_mutation)
colnames(new_genes_tumor_mutation_num) <-
  colnames(new_genes_tumor_mutation)
...
DEG_voom_can <-
  DEG_voom[with(DEG_voom, (adj.P.Val < 0.01 & abs(FC) >= 1)),]
proper_gene <-
  rownames(DEG_voom_can[order(DEG_voom_can$logFC, decreasing = T), ])
...
pheatmap(
  ...
  show_colnames = F,
  main = "DEG_SSG&only_2_3"
)
dev.off()

###Functional enrichment
up_genes <-
  rownames(DEG_voom[with(DEG_voom, (adj.P.Val < adj_p &
                                      logFC >= fold_chang)),])##Upregulated gene set
down_genes <-
  rownames(DEG_voom[with(DEG_voom, (adj.P.Val < adj_p &
                                      logFC <= -fold_chang)),])##Down-regulated gene set
...
library(clusterProfiler)
KEGG_gene <-
  enrichKEGG(down_genes_entriz,
             organism = "hsa",
             pvalueCutoff = 0.05)
GO_gene <-
  enrichGO(down_genes_entriz,
           'org.Hs.eg.db',
           ont = "BP",
           pvalueCutoff = 0.05)
pdf('down_kegg.pdf')
...
all_sample_cnv <-
  read.table(file = "CCLE_copynumber_2013-12-03.seg.txt", sep = "\t", header = T)
LUAD_cnv <-
  all_sample_cnv[all_sample_cnv$CCLE_name %in% LUAD_sample$CCLE.name, ]
colnames(LUAD_cnv) <-
  c("sample", "chr", "start", "end", "num_probes", "value")
...
for (w in 1:length(sample_cnv)) {
  ...
  for (j in 1:length(chr_name)) {
    sample_chr_cnv[[j]] <-
      sample_cnv[[w]][sample_cnv[[w]]$chr == chr_name[j], ]
  }
  names(sample_chr_cnv) <- chr_name
...
class_0 <-
  range(all_TD_size[span_size_distri$classification == 1])##-1~0.414
class_1 <-
  range(all_TD_size[span_size_distri$classification == c(1, 2)])##0.416~0.846
class_2 <-
  range(all_TD_size[span_size_distri$classification == 3])##1.85~2.71
class_3 <-
  range(all_TD_size[span_size_distri$classification == 4])##2.71~3.85

group_list <- list()
for (i in 1:length(span_size_log)) {
  group_list[[i]] <-
    c(length(which(
      class_1[1] < span_size_log[[i]] &
        span_size_log[[i]] < class_1[2]
    )), length(which(
      class_2[1] < span_size_log[[i]] &
        span_size_log[[i]] < class_2[2]
    )), length(which(
      class_3[1] < span_size_log[[i]] & span_size_log[[i]] < class_3[2]
    )))
}
...
for (i in 1:length(unique(LUAD_drug$Compound))) {
  drug_list[[i]] <-
    LUAD_drug[LUAD_drug$Compound == unique(LUAD_drug$Compound)[i], ]
}
...
for (i in c(1:24)) {
  only_1_IC <-
    drug_list[[i]][drug_list[[i]]$CCLE.Cell.Line.Name %in% only_1, ]$IC50..uM.
  only_2_3_IC <-
    drug_list[[i]][drug_list[[i]]$CCLE.Cell.Line.Name %in% only_2_3, ]$IC50..uM.
  NonTDP_group_IC <-
    drug_list[[i]][drug_list[[i]]$CCLE.Cell.Line.Name %in% NonTDP_group, ]$IC50..uM.
  only_1_2_IC <-
    drug_list[[i]][drug_list[[i]]$CCLE.Cell.Line.Name %in% only_1_2, ]$IC50..uM.
  only_1_3_IC <-
    drug_list[[i]][drug_list[[i]]$CCLE.Cell.Line.Name %in% only_1_3, ]$IC50..uM.
...
df <-
  data.frame(c(only_1_2_IC, only_1_IC), c(rep("only_1_2_IC", length(only_1_2_IC)), rep("only_1_IC", length(only_1_IC))))
colnames(df) <- c("num", "group")
...
compar <-
  list(
    c("only_2_3_IC", "only_1_IC"),
    c("only_2_3_IC", "only_1_2_IC"),
    c("only_2_3_IC", "group_1_IC")
  )
p <-
  ggboxplot(
    df,
    ...
    color = "group",
    size = 2,
    palette = c("#00AFBB", "#E7B800", "#FC4E07", "#0000CD"),
    add = "jitter",
    shape = "group"
  )
p1 <-
